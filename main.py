...
''')
    ...
    def on_factory_setup(self, factory, list_item):
        ...
        try:
            gesture = Gtk.GestureClick.new()
            gesture.connect('pressed', self.on_row_pressed, list_item)
            box.add_controller(gesture)
        except Exception:
            pass

    def on_row_pressed(self, gesture, n_press, x, y, list_item):
        # The default ListView click behavior only ever selects a row; to
        # get toggle-off-on-second-click behavior we intercept the press
        # here (this widget is the click target, so we run before the
        # ListView's own bubble-phase selection handler) and, if the row is
        # already selected, unselect it ourselves and claim the sequence so
        # the default handler doesn't just reselect it.
        if n_press != 1:
            return
        ...
        if self.selection.is_selected(pos):
            self.selection.unselect_item(pos)
    ...
    def on_map_click(self, gesture, n_press, x, y):
        ...
        if hex_code == self.selected_hex:
            # Clicking the already-selected aircraft toggles it off. Unselect
            # via the table's selection model (rather than clearing
            # selected_hex directly) so on_table_row_selected stays the
            # single place that updates selected_hex and redraws the map.
            sel_pos = self.selection.get_selected()
            if sel_pos != Gtk.INVALID_LIST_POSITION:
                self.selection.unselect_item(sel_pos)
            else:
                self.selected_hex = None
                self.selected_hex_lost_at = None
                self.map_area.queue_draw()
